        ...
        <div className='flex flex-col md:flex-row max-w-6xl lg:mx-auto m-4 p-4 rounded-lg shadow-lg bg-white lg:space-x-5'>
            <div className='w-full'>
                <p className='text-2xl font-bold mb-3 text-blue-900'>
                  ...
                  .replace(/\B(?=(\d{3})+(?!\d))/g, ",")} Rupees{listing.type === "rent" && " / month"}</p>
                <p className='flex items-center mt-6 mb-3 font-semibold '>
                    <FaMapMarkerAlt className='text-green-700 mr-1' /> 
                    {listing.address}</p>
                <div className='flex justify-start items-center space-x-4 w-[75%]'>
                    <p className='bg-red-800 w-full max-w-[200px] text-white rounded-md p-1 font-semibold text-center shadow-md'>For {listing.type === 'rent' ? "Rent" : "Sell"}</p>
                    {listing.offer && (
                        <p className='w-full max-w-[200px] bg-green-800 text-white rounded-md p-1 font-semibold text-center shadow-md'>{+listing.regularPrice - +listing.discountedPrice} Rupee discount!</p>
                    )}
                </div>
                ...
                <ul className='flex items-center space-x-2 md:space-x-10 text-sm font-semibold mb-6'>
                    ...
                    <li className='flex items-center whitespace-nowrap'>
                        <FaParking className='text-lg mr-1'/>{listing.parking ? 'With Parking Spot' : 'No Parking'}</li>
                    <li className='flex items-center whitespace-nowrap'>
                        <FaChair className='text-lg mr-1'/>{listing.furnished ? 'Furnished' : 'Not Furnished'}</li>
                </ul>
                {listing.userRef !== auth.currentUser?.uid && !contactLandlord &&(
                    <div className='mt-6'>
                        <button className='px-7 py-3 bg-blue-600 text-white font-medium text-sm uppercase rounded shadow-md hover:bg-blue-700 hober:shadow-lg focus:bg-blue-700 focus:shadow-lg
                        w-full text-center transtion duration-150 ease-in-out' 
                        onClick={()=> setContactLandlord(true)}>Contact Landlord</button>
                </div>
                )}
